...
async def create_indexes(db) -> None:
    ...
    await db.emergency_equipment.create_index([("id", 1)], unique=True)
    await db.emergency_equipment.create_index([("equipment_code", 1)], unique=True)
    await db.emergency_equipment.create_index([("area_code", 1), ("status", 1), ("readiness_percentage", 1)])
    await db.emergency_equipment.create_index([("next_inspection_date", 1)])
    await db.equipment_inspections.create_index([("equipment_id", 1), ("inspection_date", -1)])
    await db.equipment_alerts.create_index([("equipment_id", 1), ("is_acknowledged", 1), ("created_at", -1)])
