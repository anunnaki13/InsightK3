...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow trusted preview subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*preview\.local$/)) {
        return true;
      }

      // Allow alternate trusted preview subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*previewagent\.local$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@insightk3.local" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@insightk3.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
